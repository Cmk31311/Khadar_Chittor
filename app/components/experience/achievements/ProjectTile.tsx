...
const ProjectTile = ({ project, index, position, rotation, activeId, onClick }: ProjectTileProps) => {
  ...
  const isProjectSectionActive = usePortalStore((state) => state.activePortalId === "achievements");
  ...
  useEffect(() => {
    ...
    if (titleRef.current) {
      hoverAnimRef.current.to(titleRef.current.position, { y: hovered ? 0.9 : -0.7 }, 0);
    }
